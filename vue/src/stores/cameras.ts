...
export const useCameraData = defineStore('camera', {
	...
	actions: {
		async getWebcams() {
			...
			this.cameras = webcams.data.webcams.map((el: {title: string; viewCount: number, webcamId: number, status: string, lastUpdatedOn: string}) => ({
				title: el.title,
		...
		async getWebcamDetails(cameraid: number, categories?: Set<'categories' | 'images' | 'location' | 'player' | 'urls'>) {
